...
class ReviewService(
    ...
    fun reviewClass(user: User, request: ReviewGymClassRequest): GymClassReviewDto {
        val instance = gymClassInstanceRepository.findById(request.gymClassInstanceId).orElseThrow()
        val member = user.getMember(instance.gymClass.gym.id)

        val classReview = GymClassReview(
            review = request.review,
            rating = request.rating,
            gymClassInstance = instance,
            member = member
        )
